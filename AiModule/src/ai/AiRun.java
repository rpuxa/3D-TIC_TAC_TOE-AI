...
import java.util.Map;
  ...
  public class AiRun {
    ...
    static Map<StateInfo,Integer> states = new HashMap<>();
      ...
      private static int quiescent(int[] db,int[] dw,int[] t){
          while (true) {
              //Атака 4 в ряд
              int a=0;
              int b =Secondary.attack(dw,t);
              if (b!=0)
                  return 1000;

              //Защита 4 в ряд
                  b = Secondary.attack(db, t);
                  if (b != 0)
                      a = b;
              ...
              if (a==0)
                  ...
                  System.arraycopy(Secondary.auth_d(dw.clone(), t, a), 0, dw, 0, 77);
                t[a]++;
                  //Атака 4 в ряд
                  a=0;
                   b =Secondary.attack(db,t);
                  ...
                  if (a==0)
                      ...
                      System.arraycopy(Secondary.auth_d(db.clone(), t, a), 0, db, 0, 77);
                    t[a]++;
                  }
          }
      }
      ...
      private static int analyzeMaxDepth(int[] db,int[] dw,int[] t) {
          ...
          //int a=quiescent(db.clone(),dw.clone(),t.clone());
        //  if (a!=0)
        //      return a;
    ...
    public static int analyze(int[] db, int[] dw, int[] t, int depth, int maxDepth,int alfa,int[] move) {
        int result;
        int lastMove = 0;
        if (depth == 0) {
            int min = 10000;
            ...
            if (t[i]<4)
            {
                ...
                if (states.get(state)==null) {
                    result = analyze(db.clone(), dw.clone(), t.clone(), depth + 1, maxDepth, min,move);
        ...
        if ((depth)%2==1) {
                ...
                if (t[i]<4)
            {
                ...
                StateInfo state = new StateInfo(db, dw, depth);
                if (states.get(state)==null) {
                    result = analyze(db.clone(), dw.clone(), t.clone(), depth + 1, maxDepth, max,move);
                    states.put(state, result);
                }
                else
                   result = states.get(state);
        ...
        if (depth == 2) {
            ...
            if ((depth)%2==0) {
                ...
                StateInfo state = new StateInfo(db, dw, depth);
               if (states.get(state)==null) {
                    result = analyze(db.clone(), dw.clone(), t.clone(), depth + 1, maxDepth, alfa,move);
                    states.put(state, result);
                }
               else
                   result = states.get(state);
